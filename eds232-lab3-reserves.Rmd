...
**Question**: What part of the study area is the 13th vegetation class found in? For instance, is it in the south-eastern part of the study area?\
...
**Question**: Can you verify that all of the targets were met in the prioritization?\
**Answer**: Yes, the lowest value in `s1` for column `relative_held` is `r round(min(eval_feature_representation_summary(p1, s1[, "solution_1"])[5]), 4)`. This is above 5%, meaning that all of the targets were met in the prioritization.\
...
**Answer**: Yes, the solutions with more planning units have a greater costs. This makes sense because the solution cost is a sum of the cost for each planning unit, so more units will likely result in a higher costs.\
...
**Answer**: The first solution costs less than the second solution because there are fewer planning units included in the first solutions. This is because the second solution includes the additional constraint of including all planning units that are already covered by a protected area (i.e. "locked-in").\
...
**Answer**: The third solution costs less than the fourth solution because for the forth solution we have added a second constraint of excluding all "degraded" (i.e. "locked-out) planning units. The costs of the degraded units is likely lower than the costs of non-degraded units, so it makes sense that the overall cost of the solution would be higher when we force the solution to exlude these units.\
...
**Answer**: The cost of the planning units selected in `s4` is \$`r round(eval_cost_summary(p1, s4[, "solution_1"])[[2]], 2)` and the cost of the planning units selected in `s5` is \$`r round(eval_cost_summary(p1, s5[, "solution_1"])[[2]], 2)`. The fifth solution costs more than the fourth solutions because we have added yet another constraint (pick planning units that are more clustered together), which forces the solution to pick planning units with a higher cost than solution 4.\

**Question**: Try setting the penalty value to 0.000000001 (i.e. 1e-9) instead of 0.001. What is the cost of the solution now? Is it different from the fourth solution (`s4`) (hint: try plotting the solutions to visualize them)? Is this a useful penalty value? Why (or why not)?\
```{r, out.width = "65%"}
# make prioritization problem
p5_rds <- file.path(dir_data, "p5.rds")
if (!file.exists(p5_rds) | TRUE){
...
**Answer**: The cost of the new penalty solution is \$`r round(eval_cost_summary(p1, s5[, "solution_1"])[[2]], 2)`, which is very close to to the cost of the fourth solution (\$`r round(eval_cost_summary(p1, s4[, "solution_1"])[[2]], 2)`). When plotted the maps also look similar (with slight differences). Because this solution is so similar to `s4`, I do not think this is a helpful penalty value.\


**Question**: Try setting the penalty value to 0.5. What is the cost of the solution now? Is it different from the fourth solution (`s4`) (hint: try plotting the solutions to visualize them)? Is this a useful penalty value? Why (or why not)?\
```{r, out.width = "65%"}
# make prioritization problem
p5_rds <- file.path(dir_data, "p5.rds")
if (!file.exists(p5_rds) | TRUE){
